<template>
  <section class="vh-100 container">
    ...
    <!-- <div class="input-group mt-4">
      ...
      <button type="button" class="btn btn-primary" @click="getCurrentLocation">
    ...
    <div class="container py-5 h-100 justify-content-center d-flex">
      <div class="d-flex">
        ...
        <div class="row d-flex justify-content-center align-items-center mt-4">
          <div v-if="data.name" class="main-width" v-show="data.length != 0">
            <div class="card" style="color: #4b515d; border-radius: 35px">
              <div class="card-body p-4">
                ...
                >
                  <div>
                    <img src="../assets/wind-beaufort-1.svg" />
        ...
        <div class="d-flex p-4" v-for="(item, i) in forecasteData">
          <div
                    ...
                    <div>
                      <img
                        :src="
                          images.find(
                            (image) =>
                              image.alt === forecasteData[i].weather[0].main
                          )?.url
                        "
                        width="100px"
                      />
                      ...
                      <span class="ms-1">
                        {{
                          new Intl.DateTimeFormat("en-US", {
                            weekday: "short",
                          }).format(new Date(forecasteData[i].dt_txt))
                        }}</span
                      >
                    </div>
                  </div>
...
let lat = "40.25631";
let lon = "40.22289";
const CURRENTWEATHERAPI = `https://api.openweathermap.org/data/2.5/weather?lat={0}&lon={1}&appid=${process.env.VUE_APP_API_OMAP_KEY}`;
...
const weather16days = `https://api.openweathermap.org/data/2.5/forecast/daily?lat={0}&lon={1}&cnt=7&appid=${process.env.VUE_APP_API_OMAP_KEY}`;
...
export default {
  ...
  methods: {
    ...
    getPosition(position) {
      ...
      axios
        .get(CURRENTWEATHERAPI.replace("{0}", lat).replace("{1}", lon))
        .then((res) => {
      ...
      axios
        .get(FORECASTFIVEDAYAPI.replace("{0}", lat).replace("{1}", lon))
        .then((res) => {
          console.error(res.data);
          const watherList = {};
    ...
    // forecasteFiveDay() {
    //   axios.get(FORECASTFIVEDAYAPI).then((res) => {
